...
async function fetchTweetWithRetry(id: string, maxRetries = 3, currentAttempt = 1): Promise<Tweet | undefined> {
  try {
    const tweet = await getTweet(id)
    return tweet
  } catch (error) {
    console.error(`Tweet fetch error (attempt ${currentAttempt}/${maxRetries}):`, error)
